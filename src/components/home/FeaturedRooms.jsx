...
import { FaBed, FaUsers, FaArrowRight } from 'react-icons/fa';
...
const FeaturedRooms = () => {
  ...
  const fetchRooms = async () => {
    try {
      ...
      const fetchedRooms = response.data.data.slice(0, 3);
      setRooms(fetchedRooms);
      
      // Initialiser les index d'images pour chaque room
  ...
  // Slideshow automatique pour chaque room
  useEffect(() => {
      ...
      setImageIndexes(prevIndexes => {
        ...
        rooms.forEach(room => {
          if (room.photos && room.photos.length > 1) {
            newIndexes[room.id] = (prevIndexes[room.id] + 1) % room.photos.length;
          }
        });
        return newIndexes;
      });
      ...
      <div className="container-custom relative z-10 px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-10 md:mb-12">
          {rooms.map((room) => {
            ...
            const currentImageIndex = imageIndexes[room.id] || 0;
                ...
                <div className="relative h-56 sm:h-64 md:h-72 bg-accent overflow-hidden">
                  {room.photos && room.photos.length > 0 ? (
                    <>
                      ...
                      {/* ✅ Dots pour le slideshow - Affichés seulement s'il y a plusieurs photos */}
