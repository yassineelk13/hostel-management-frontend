...
const Navbar = () => {
  ...
  // ✅ Packages et Rooms → pages dédiées
  const navLinks = [
    { name: 'Home',     path: '/',        anchor: 'hero',     type: 'anchor' },
    { name: 'Packages', path: '/packs',   anchor: 'packs',    type: 'route'  }, // ✅ page
    { name: 'Rooms',    path: '/rooms',   anchor: 'chambres', type: 'route'  }, // ✅ page
  ...
  const handleNavClick = (e, link) => {
    ...
    // ✅ ROUTE : navigation directe vers la page
    if (link.type === 'route') {
      e.preventDefault();
      navigate(link.path);
      return;
    ...
    if (link.type === 'anchor') {
      ...
      if (location.pathname === '/') {
        ...
        navigate('/');
        setTimeout(() => scrollToSection(link.anchor), 150);
      }
    }
  };
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6">
        <div className="flex justify-between items-center h-16 sm:h-18 md:h-20">
          ...
          <div className="hidden lg:flex items-center space-x-1 xl:space-x-2">
            {navLinks.map((link) => (
              <Link
                ...
                className={`relative px-3 xl:px-4 py-2 font-semibold transition-all duration-300 rounded-lg group text-sm xl:text-base whitespace-nowrap ${
                  isActive(link) ? 'text-primary' : 'text-dark hover:text-primary'
                }`}
              >
                {link.name}
            ...
            <Link to="/booking" className="ml-2 xl:ml-4 group relative overflow-hidden flex-shrink-0">
          ...
          <div className="py-3 sm:py-4 space-y-1">
            {navLinks.map((link, index) => (
              <Link
                ...
                to={link.path}
                onClick={(e) => handleNavClick(e, link)}
                className={`block py-2.5 sm:py-3 px-3 sm:px-4 font-semibold rounded-lg transition-all duration-300 text-sm sm:text-base ${
                  isActive(link)
